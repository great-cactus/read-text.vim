...
export async function main(denops: Denops): Promise<void> {
  async function getConfig(): Promise<Config> {
    return {
      ttsProvider: await vars.g.get(denops, "read_text_tts_provider", "voicevox") as Config["ttsProvider"],
      ...
      readingRulesMode: await vars.g.get(denops, "read_text_reading_rules_mode", "extend") as Config["readingRulesMode"],
    };
  }
  ...
  async function getReadingRulesConfig(): Promise<ReadingRulesConfig> {
    return {
      ...
      mode: await vars.g.get(denops, "read_text_reading_rules_mode", "extend") as ReadingRulesConfig["mode"],
      customRules: [],
    };
  }
  ...
  async function executeReading(
    ...
    try {
      ...
      await denops.cmd(`echohl ErrorMsg | echo '${operationName} failed: ${message}' | echohl None`);
    }
  }

  denops.dispatcher = {
    readFromCursor(speakerId?: unknown): Promise<void> {
      const id = typeof speakerId === "number" ? speakerId : undefined;
      return executeReading(() => textExtractor.extractFromCursor(), "ReadFromCursor", id);
    },

    readSelection(speakerId?: unknown): Promise<void> {
      const id = typeof speakerId === "number" ? speakerId : undefined;
      return executeReading(() => textExtractor.extractSelection(), "ReadSelection", id);
    },

    readLine(speakerId?: unknown): Promise<void> {
      const id = typeof speakerId === "number" ? speakerId : undefined;
      return executeReading(() => textExtractor.extractCurrentLine(), "ReadLine", id);
    },
    ...
    pauseReading(): boolean {
      return currentEngine?.pause() ?? false;
    },

    resumeReading(): boolean {
      return currentEngine?.resume() ?? false;
    },

    togglePauseReading(): boolean {
      if (!currentEngine) return false;
      return currentEngine.isPaused() ? currentEngine.resume() : currentEngine.pause();
    },
